...
int main()
{
  ...
  */
  ...
  // glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, destinationTexture, 0);
  ...
  // unsigned int *data = {new unsigned int[nb_pixel]{}};
  // for (int i = 0; i < nb_pixel; ++i)
  // {
  //     data[i] = 128; // static_cast<unsigned int>(dist(mt)) % 255;
  //     // std::cout << (int)data[i] << std::endl;
  // }

  // unsigned int *result = {new unsigned int[nb_pixel * 3]{}};
  // for (int i = 0; i < nb_pixel; ++i)
  // {
  //     result[i] = 1; // static_cast<unsigned int>(dist(mt)) % 255;
  // }
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    // glActiveTexture(GL_TEXTURE0);
    glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer);
    glUniform1f(glGetUniformLocation(shaderProgramId, "u_source_texture"), 0);
    ...
    // here draw some geometry

    // --------------------------------------

    //  glClearColor(1.0f, 1.D0f, 0.0f, 1.0f);
    //  glClear(GL_COLOR_BUFFER_BIT);

    // binding framebuffer
    // glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer);
    // glDrawBuffer(colorAttachment); // not sure if special color attachment needed
